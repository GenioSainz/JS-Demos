...
class Particle{
  ...
  constructor(position, velocity, pObj,{radius=20,color=[0,0,0],velAngle=5,velMax=4,velMin=2}={}){
    ...
    returnToXY(x,y){
        ...
        if( boolX || boolY ){
            ...
            var m  = this.p.random(this.velMin,this.velMax);
            var d  = this.p.random(-this.velAngle,this.velAngle)*Math.PI/180;
            var vx = m*Math.cos( d );
            var vy = m*Math.sin( d );
