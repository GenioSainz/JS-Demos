...
function plot3D(id){
    ...
    for(let i=0;i<towersXY.length-1;i++){
        ...
        catenarysData.xCi.push(xai);
        catenarysData.yCi.push(yai);
        catenarysData.zCi.push(zi );

        catenarysData.xCf.push(xaf)
        catenarysData.yCf.push(yaf)
        catenarysData.zCf.push(zf )
...
function catenary3D(x1,y1,z1,x2,y2,z2,c){
...
function updateTraces(){
    ...
    var vec = tf.linspace(1,200,catenaryPoints).arraySync();
    var vec_ = [];

    for(let i=0;i<catenaryTracesIndex.length;i++){
         vec_.push(vec)
         catenary3D(xai,yai,zi,xaf,yaf,zf,cCatenary) 
    };

    var updateTraces = {'x': vec_, 'y':vec_,'z':vec_};
